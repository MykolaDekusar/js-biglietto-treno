...
if ((userChoice === pari) || (userChoice === dispari)) {  //verifico che l'utente abbia inserito il valore valido
    ...
    userNumber = Math.round(Number(prompt("Inserisci un numero compreso tra 1 e 9")));//trasformo la stringa inserita dall'utente in un numero intero
    if (!(userNumber < 1) && !(userNumber > 9) && !(isNaN(userNumber))) {  //verifico che l'utente abbia inserito un numero valido
        ...
        if ((totNumber % 2 === 0 && userChoice === pari) || (totNumber % 2 !== 0 && userChoice === dispari)) {
